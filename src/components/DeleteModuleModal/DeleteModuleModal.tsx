...
const DeleteModuleModal = ({ close, updateCourse, module }: Props) => {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    await axios.delete(`${import.meta.env.VITE_API_URL}/module/${module.id}`, {
      headers: {
          ...
          >
            ...
            <fieldset className='flex w-full flex-col'>
              <label htmlFor='title' className='px-4 py-2'>
                Enter Title: {module.title} to DELETE
              </label>
              <input
                type='text'
                name='title'
