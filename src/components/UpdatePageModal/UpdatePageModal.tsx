...
const UpdatePageModal = ({ close, updateModule, page }: Props) => {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const attachmentsUrls: string[] = page.attachmentsUrls;

    await axios.put(
      `${import.meta.env.VITE_API_URL}/page/${page.id}`,
  ...
  useEffect(() => {
    titleInput.current!.value = page.title;
    markdownContentInput.current!.value = page.markdownContent;
    instructorNotesInput.current!.value = page.instructorNotes;
  }, []);
    ...
    <>
      <div className='fixed bottom-0 left-0 right-0 top-0 bg-slate-500 bg-opacity-50'>
        <div className='fixed left-1/2 top-1/2 flex w-1/3 -translate-x-1/2 -translate-y-1/2 flex-col rounded-md bg-slate-50 p-8'>
          ...
          >
            <h1 className='text-4xl font-bold text-primary-500'>Edit Page</h1>
